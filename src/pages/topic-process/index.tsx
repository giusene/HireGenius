...
const TopicProcess: React.FC = () => {
	...
	const handleInterviewerSelect = async (selectedInterviewer: Interviewer) => {
		...
		setInterviewDetails({ ...interviewOptions, interviewer: selectedInterviewer });
		...
		if (interviewDetails) {
			...
			const prompt = `
      Immagina di essere l'esaminatore ${interviewDetails.interviewer.name}. ${interviewDetails.interviewer.longBio}.
      Devi condurre un colloquio tecnico ${interviewDetails.level}.
      Ponimi ${interviewDetails.numQuestions} domande tecniche sul tema "${interviewDetails.topic}", di difficoltà crescente.
    `;
