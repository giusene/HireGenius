...
const QuizCard: React.FC<QuizCardProps> = ({ interviewSession, onDelete, setSelectedInterviewSession }) => {
	...
	return (
		<div className={`${style.quizCardContainer} ${showFeedback && style.selectedCard} `}>
			<div className={style.quizCard}>
				...
				<div className={style.content}>
					...
					<h4>
						Punteggio: <span>{interviewSession.evaluationResult.globalEvaluation.points}/100</span>
					</h4>
					<h4>
						Intervistatore: <span>{interviewSession.interviewDetails.interviewer.name}</span>
					</h4>
				</div>
				<div className={style.buttons}>
					<ActionButton className='round' icon={DeleteIcon} onClick={handleDelete} />
					<ActionButton className='round' icon={showFeedback ? arrowUp : arrowSelect} onClick={() => setShowFeedback(!showFeedback)} />
				</div>
			</div>
			{showFeedback && (
				<>
			...
			<ConfirmDeletePopup isOpen={showPopup} onConfirm={confirmDelete} onCancel={cancelDelete} />
		</div>
	);
};
